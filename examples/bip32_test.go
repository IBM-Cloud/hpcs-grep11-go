...
// => base test covers private->private and private->public key derivation
func Example_bip32_Base() {
	...
	var privateKey [maxDepth][maxChild]*pb.KeyBlob
	var privateChainCode [maxDepth][maxChild][]byte
	var publicKey [maxDepth][maxChild]*pb.KeyBlob
	...
	fmt.Println("Depth2: Generated internal and external wallet chains successfully")
	...
	fmt.Println("Sign and Verify operations starting...")
	...
	fmt.Println("Sign and Verify operations completed successfully")

	fmt.Println("SignSingle and VerifySingle operations starting...")
	...
	fmt.Println("SignSingle and VerifySingle operations completed successfully")
	...
	// Derived Key type=CkBIP0032PRV2PRV index=0
	// Derived Key type=CkBIP0032PRV2PUB index=0
	// Depth2: Generated internal and external wallet chains successfully
	// Depth3: Generating wallet addresses...
	// Derived Key type=CkBIP0032PRV2PRV index=0
	// Derived Key type=CkBIP0032PRV2PUB index=0
	// Depth3: Generated external and internal addresses successfully.
	// Sign and Verify operations starting...
	// Data signed
	// Signature verified
	// Data signed
	// Signature verified
	// Data signed
	// Signature verified
	// Sign and Verify operations completed successfully
	// SignSingle and VerifySingle operations starting...
	// Data signed
	// Signature verified
	// Data signed
	// Signature verified
	// Data signed
	// Signature verified
	// SignSingle and VerifySingle operations completed successfully
}

// Example_bip32_KeyDerivation covers private->private and public->public key derivations
func Example_bip32_KeyDerivation() {
	...
	if err != nil {
		fmt.Println("Deriving keys from private to private is currently not supported")
	} else {
	...
	if err != nil {
		...
		fmt.Println("Derived key from public to public successfully")
	}
	...
	// Derived key from public to public successfully
}
...
func Example_bip32_Cross_SignVerify() {
	...
	var privateKeyW [3]*pb.KeyBlob
	var privateKeyChainCodeW [3][]byte
	var publicKeyW [3]*pb.KeyBlob
	...
	signData1 := []byte("This data needs to be signed")
	...
	signData2 := []byte("This data needs to be signed")
	...
	signData3 := []byte("This data needs to be signed")
...
func bip32DeriveKey(cryptoClient pb.CryptoClient, deriveType pb.BTCDeriveParm_BTCDeriveType, childKeyIndex uint64, baseKey *pb.KeyBlob, chainCode []byte) (*pb.KeyBlob, []byte) {
	ecParameters, err := asn1.Marshal(util.OIDNamedCurveSecp256k1)
	if err != nil {
		panic(fmt.Errorf("Derived Child Key request error: %s", err))
	}
	...
	return deriveKeyResponse.NewKey, deriveKeyResponse.CheckSum
}

func bip32SignAndVerify(cryptoClient pb.CryptoClient, privateKey *pb.KeyBlob, publicKey *pb.KeyBlob) bool {
...
func bip32SignAndVerifySingle(cryptoClient pb.CryptoClient, privateKey *pb.KeyBlob, publicKey *pb.KeyBlob) bool {
	signData := []byte("This data needs to be signed")
